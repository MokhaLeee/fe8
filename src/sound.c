...
static Proc_t *sMusicProc1;
static Proc_t *sMusicProc2;

struct MusicProc {
    PROC_HEADER;
    /*0x2A*/ i16 filler2A[16];
    /*0x4A*/ i16 songId;
    /*0x4C*/ i16 delayCounter; // 16
    /*0x4E*/ i16 unk4E; // 17
    /*0x50*/ i16 filler50[2];
...
static void func_08002788(Proc_t *proc);
...
static void func_080024F0(Proc_t *proc)
{
...
void DelaySong_OnLoop(Proc_t *proc)
{
    struct MusicProc *mproc = (struct MusicProc *)proc;
    mproc->delayCounter--;
...
void PlaySongCore(int songId, struct MusicPlayerInfo *player)
{
    if (songId < 128)
    {
        func_080028FC(songId);
        func_080A3F08(0, songId);
    }
...
void func_0800296C(Proc_t *proc)
{
    struct MusicProc *mproc = (struct MusicProc *)proc;
...
void func_080029BC(Proc_t *proc)
{
    struct MusicProc *mproc = (struct MusicProc *)proc;
    if (mproc->unk5C > 0)
    {
...
{
    PROC_SLEEP(1),
    PROC_CALL(func_0800296C),
    PROC_SLEEP(1),
    PROC_CALL(func_080029BC),
    PROC_SLEEP(8),
    PROC_LABEL(0),
    PROC_SLEEP(0),
    PROC_END,
...
void func_080029E8(int songId, int b, int c, int d, Proc_t *parent)
{
