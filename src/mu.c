...
// ProcScr_MUStepSound
static struct ProcScr CONST_DATA sProcScr_MUStepSound[] = {
...
static struct ProcScr CONST_DATA sProcScr_MUFogBumpFx[] = {
...
struct ProcScr CONST_DATA ProcScr_MoveUnit[] = {
    PROC_NAME("MOVEUNIT"),
    PROC_MARK(4),
...
// ProcScr_MUBlinkingEffect
struct ProcScr CONST_DATA ProcScr_MUBlinkEffect[] = {
...
static struct ProcScr CONST_DATA sProcScr_MUPixelEffect[] = {
    PROC_REPEAT(MU_PixelEffect_OnLoop),
    PROC_END
};

struct ProcScr CONST_DATA ProcScr_MU_89A2CF8[] = {
    PROC_SLEEP(8),
...
static struct ProcScr CONST_DATA sProcScr_MUCritFlash[] = {
...
static struct ProcScr CONST_DATA sProcScr_MUHitFlash[] = {
    PROC_SLEEP(17),
...
struct MUProc* MU_Create(Unit_t* unit) {
    ...
    if (unit->flags & UNIT_FLAG_IN_BALLISTA) {
        Trap_t* blst = GetTrap(unit->ballistaId);
...
static struct MUProc* MU_CreateInternal(u16 x, u16 y, u16 jid, int objTileId, unsigned palId) {
    ...
    if (FindProc(ProcScr_MoveUnit))
        soundTimer = -2;

    proc = SpawnProc(ProcScr_MoveUnit, PROC_TREE_5);
    ...
    ap = StartSpriteAnim(MU_GetAnimationByClassId(jid), 10);
    SetSpriteAnimId(ap, MU_FACING_SELECTED);
    ...
    ap->img = MU_GetGfxBufferById(config->muIndex);
    ap->oam2 = ((config->paletteIndex & 0xF) << 12) + config->objTileIndex + proc->objPriorityBits;
...
void MU_SetFacing(struct MUProc* proc, int facingId) {
    ...
    if (facingId == MU_FACING_STANDING)
        func_08027068(proc->muIndex, proc->pGfxVRAM);
    else
        SetSpriteAnimId(proc->sprite_anim, proc->facingId);
}
...
void MU_SetDefaultFacing_Auto(void) {
    struct MUProc* proc = FindProc(ProcScr_MoveUnit);
...
int MU_Exists(void) {
    return FindProc(ProcScr_MoveUnit) ? TRUE : FALSE;
}
...
int MU_IsAnyActive(void) {
    asm(
        ...
        "push {lr}\n"
        "movs r3, #0\n"
        "ldr r0, .L08078764  @ sMUConfigArray\n"
        "adds r2, r0, #0\n"
        "adds r2, #0x48\n"
        "adds r1, r0, #0\n"
    ".L08078744:\n"
    ...
    ".L08078754:\n"
        ...
        "b .L0807876A\n"
        ".align 2, 0\n"
    ".L08078764: .4byte sMUConfigArray\n"
    ".L08078768:\n"
        "movs r0, #1\n"
    ".L0807876A:\n"
        "pop {r1}\n"
...
void MU_StartStepSfx(int soundId, int b, int hPosition) {
    ...
    proc = FindProc(sProcScr_MUStepSound);

    if (!proc)
        proc = SpawnProc(sProcScr_MUStepSound, PROC_TREE_3);
...
void MU_StartFogBumpFx(int x, int y) {
    ...
    ap = StartSpriteAnim(gUnknown_089A8EF8, 2);

    ap->oam2 = 0x1180;
    SetSpriteAnimId(ap, MU_FACING_LEFT);
...
u8 MU_IsFogBumpFxActive(void) {
    return FindProc(sProcScr_MUFogBumpFx) ? TRUE : FALSE;
}
...
static void MU_State_DuringMovement(struct MUProc* proc) {
    ...
    if (proc->boolAttractCamera && !FindProc(ProcScr_CameraPan)) {
...
void MU_AllDisable(void) {
    LockEachMarkedProc(4);
}

void MU_AllEnable(void) {
    ReleaseEachMarkedProc(4);
}
...
void MU_AllRestartAnimations(void) {
    ...
    for (i = 0; i < MU_MAX_COUNT; ++i) {
        if (sMUConfigArray[i].muIndex) {
            sMUConfigArray[i].pMUProc->sprite_anim->clock    = 0;
            sMUConfigArray[i].pMUProc->sprite_anim->clock_interval_q8 = 0x100;
        }
    }
}
...
static const void* MU_GetSheetGfx(struct MUProc* proc) {
    return gMMSDataTable[proc->displayedClassId - 1].img;
}

static const void* MU_GetAnimationByClassId(u16 jid) {
    return gMMSDataTable[jid - 1].anim;
}
...
void MU_StartDeathFade(struct MUProc* muProc) {
    ...
    proc = SpawnProc(ProcScr_MUDeathFade, muProc);
    ...
    muProc->sprite_anim->sprite_layer = 0xD;
...
void MU_StartBlinkEffect(struct MUProc* muProc) {
    ...
    proc = SpawnProc(ProcScr_MUBlinkEffect, muProc);
...
void MU_StartPixelEffect(struct MUProc* muProc) {
    ...
    proc = SpawnProc(sProcScr_MUPixelEffect, muProc);
...
void MU_8079858(struct MUProc* muProc) {
    ...
    func_08013928(
        ...
        0x15, 8, (Proc_t*) muProc
    );

    proc = SpawnProc(ProcScr_MU_89A2CF8, PROC_TREE_3);
...
void MU_StartActionAnim(struct MUProc* proc) {
    ...
    proc->sprite_anim->clock_interval_q8 = 0x100;

    SetupFutureCall(MU_EndSelectionApAnim, (int) proc->sprite_anim, 30);
}

static void MU_EndSelectionApAnim(int argAp) {
    SpriteAnim_t* ap = (SpriteAnim_t*) argAp;

    ap->clock    = 0;
    ap->clock_interval_q8 = 0;
}
...
void MU_StartDelayedFaceTarget(struct MUProc* proc) {
    ...
    SetupFutureCall(MU_EndRefaceApAnim, (int) proc->sprite_anim, 30);
}
...
static void MU_EndRefaceApAnim(int argAp) {
    ...
    SetBattleAnimActorFacing(
        actor1, actor2,
        GetSpellAssocFacing(gMapAnimData.actors[0].pBattleUnit->weapon_before)
    );
...
void MU_StartFastMoveAnim(struct MUProc* proc) {
    ...
    SetupFutureCall(MU_EndFasterApAnim, (int) proc->sprite_anim, 20);
}
...
void MU_StartCritFlash(struct MUProc* muProc, int flashType) {
    ...
    proc = SpawnProc(sProcScr_MUCritFlash, muProc);
...
void MU_StartHitFlash(struct MUProc* muProc, int flashType) {
    ...
    func_08013928(
        gPal + 0x10 * (0x10 + muProc->pMUConfig->paletteIndex),
        0x15, 0x14, (Proc_t*) muProc
    );

    proc = SpawnProc(sProcScr_MUHitFlash, muProc);
...
void MU_AllForceSetMaxMoveSpeed(void) {
    ForEachProc(ProcScr_MoveUnit, MU_ForceSetMaxMoveSpeed);
}
...
void MU_SetSpecialSprite(struct MUProc* proc, int displayedClassId, const u16* palette) {
    proc->sprite_anim->clock = 0;
    proc->sprite_anim->clock_interval_q8 = 0;
...
void MU_SortObjLayers(void) {
    ...
    for (i = 0; i < count; ++i)
        procs[i]->sprite_anim->sprite_layer = 10 - i;
}
