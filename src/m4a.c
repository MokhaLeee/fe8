...
void m4aSoundInit(void)
{
    ...
    CpuCopy32((void *)((i32)SoundMainRAM & ~1), SoundMainRAM_Buffer, sizeof(SoundMainRAM_Buffer));
...
{
    i32 trackCount = mplayInfo->trackCount;
...
void SoundInit(struct SoundInfo *soundInfo)
{
    ...
    REG_DMA1SAD = (i32)soundInfo->pcmBuffer;
    REG_DMA1DAD = (i32)&REG_FIFO_A;
    REG_DMA2SAD = (i32)soundInfo->pcmBuffer + PCM_DMA_BUF_SIZE;
    REG_DMA2DAD = (i32)&REG_FIFO_B;
...
void SoundClear(void)
{
    ...
    while (i > 0)
    {
        ...
        i--;
        chan = (void *)((i32)chan + sizeof(struct SoundChannel));
    }
    ...
    if (chan)
    {
        ...
        while (i <= 4)
        {
            ...
            i++;
            chan = (void *)((i32)chan + sizeof(struct CgbChannel));
        }
    }
...
void FadeOutBody(struct MusicPlayerInfo *mplayInfo)
{
    ...
    {
        if ((i16)(mplayInfo->fadeOV -= (4 << FADE_VOL_SHIFT)) <= 0)
        {
            i = mplayInfo->trackCount;
            track = mplayInfo->tracks;
...
void TrkVolPitSet(struct MusicPlayerInfo *mplayInfo, struct MusicPlayerTrack *track)
{
    ...
    {
        i32 bend = track->bend * track->bendRange;
        i32 x = (track->tune + bend)
              * 4
              + (track->keyShift << 8)
              + (track->keyShiftX << 8)
              + track->pitX;
...
void CgbSound(void)
{
    ...
    for (ch = 1, channels = soundInfo->cgbChans; ch <= 4; ch++, channels++)
    {
        ...
        else if (channels->statusFlags & SOUND_CHANNEL_SF_IEC)
        {
            ...
            if ((i8)(channels->pseudoEchoLength & mask) <= 0)
            {
            oscillator_off:
                CgbOscOff(ch);
        ...
        envelope_step_repeat:
            if (channels->envelopeCounter == 0)
            {
                ...
                if ((channels->statusFlags & SOUND_CHANNEL_SF_ENV) == SOUND_CHANNEL_SF_ENV_RELEASE)
                {
                    ...
                    if ((i8)(channels->envelopeVolume & mask) <= 0)
                    {
                ...
                else if ((channels->statusFlags & SOUND_CHANNEL_SF_ENV) == SOUND_CHANNEL_SF_ENV_DECAY)
                {
                    ...
                    channels->envelopeVolume--;
                    envelopeVolume = (i8)(channels->envelopeVolume & mask);
                    sustainGoal = (i8)(channels->sustainGoal);
...
void m4aMPlayPitchControl(struct MusicPlayerInfo *mplayInfo, u16 trackBits, i16 pitch)
{
    i32 i;
    u32 bit;
    struct MusicPlayerTrack *track;
...
void m4aMPlayPanpotControl(struct MusicPlayerInfo *mplayInfo, u16 trackBits, i8 pan)
{
    i32 i;
    u32 bit;
    struct MusicPlayerTrack *track;
