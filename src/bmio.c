...
extern const ProcScr_t ProcScr_BMapMain[]; // ProcScr_BMapMain
...
void MapMain_ResumeFromPhaseIdle(ProcPtr mapMain);
void MapMain_ResumeFromAction(ProcPtr mapMain);
void MapMain_ResumeFromBerserkPhase(ProcPtr mapMain);
void MapMain_ResumeFromArena(ProcPtr mapMain);
void MapMain_ResumeFromPhaseSwitch(ProcPtr mapMain);

// TODO: figure out if those variables should really belong to EWRAM_DATA
EWRAM_DATA union WeatherEffectData sWeatherEffect = {};
EWRAM_DATA union GradientEffectData sGradientEffect = {};

CONST_DATA ProcScr_t ProcScr_BmVSync[] =
{
...
CONST_DATA ProcScr_t ProcScr_MapTask[] =
{
    PROC_NAME("MAPTASK"),
    PROC_END_DUPS,
    PROC_MARK(PROC_MARK_1),
...
static CONST_DATA ProcScr_t sProc_DelayedBMapDispResume[] = { // ProcScr_GameGfxUnblocker
    PROC_SLEEP(0),
...
void BMapVSync_UpdateMapPalAnimations(struct BMVSyncProc* proc) {
    ...
    CpuCopy16(
        ...
        proc->tilePalAnimCurrent->color_first + (0x10 * BM_BGPAL_6) + gPal,
        proc->tilePalAnimCurrent->color_count*2
    );
...
void StartBmVSync(void) {
    BMapVSync_InitMapAnimations(
        SpawnProc(ProcScr_BmVSync, PROC_TREE_VSYNC));
...
void SetWeather(int weather) {
    gPlaySt.chapterWeatherId = weather;
...
void InitPlayConfig(int isDifficult, i8 unk) {
...
void StartChapter(ProcPtr gamectrl_proc) {
    int i;
 ...
 */
void ResumeChapterFromSuspend(ProcPtr gamectrl_proc) {
    ProcPtr mapMain;
    ...
    mapMain = StartMapMain(gamectrl_proc);

    gBmSt.camera.x = func_08015A40(16 * gBmSt.playerCursor.x);
    gBmSt.camera.y = func_08015A6C(16 * gBmSt.playerCursor.y);
...
ProcPtr StartMapMain(ProcPtr parent) {
    Proc_t* result = SpawnProc(ProcScr_BMapMain, PROC_TREE_2);

    result->ptr_54 = parent;
    ((Proc_t*) parent)->proc_lockCnt++;
...
void EndMapMain(void) {
    ...
    mapMain = FindProc(ProcScr_BMapMain);
    ((Proc_t*) mapMain->ptr_54)->proc_lockCnt--;
