...
void Loop6C_WaitForSelectPress(Proc_t *proc)
{
...
struct ProcScr ProcScr_DebugPrintWithProc[] = {
...
void DebugPrint(int x, int y, int width, const char *text)
{
    ...
    proc = (struct DebugPrintProc *)SpawnProc(ProcScr_DebugPrintWithProc, (Proc_t *)PROC_MARK_3);
    proc->x = x;
    proc->y = y;
    proc->text = text;
    proc->width = width;
...
{
  EndMapMain();
  gPlaySt.chapter = func_0801C650(menuItemProc->itemNumber);
