...
EWRAM_DATA Unit_t gUnitArrayBlue[62]  = {}; // Player units
EWRAM_DATA Unit_t gUnitArrayRed[50]   = {}; // Red units
EWRAM_DATA Unit_t gUnitArrayGreen[20] = {}; // Green units
EWRAM_DATA Unit_t gUnitArrayPurple[5] = {}; // Purple units
...
Unit_t* CONST_DATA gUnitLookup[0x100] =
{
...
{
    u8 id = unit->id;
    CpuFill16(0, unit, sizeof(Unit_t));
    unit->id = id;
}
...
Unit_t* GetFreeBlueUnit(const struct UnitInfo* info)
{
    int i = 1, last = 0x40;
...
inline int GetUnitMaxHp(Unit_t* unit)
{
    return unit->maxHP + GetItemHpBonus(GetUnitEquippedWeapon(unit));
}

inline int GetUnitCurrentHp(Unit_t* unit)
{
...
inline int GetUnitPower(Unit_t* unit)
{
    return unit->pow + GetItemPowBonus(GetUnitEquippedWeapon(unit));
}

inline int GetUnitSkill(Unit_t* unit)
{
...
inline int GetUnitSpeed(Unit_t* unit)
{
    int item = GetUnitEquippedWeapon(unit);
...
inline int GetUnitChibiFid(Unit_t* unit)
{
...
inline int GetUnitLeaderPid(Unit_t* unit)
{
...
inline void SetUnitLeaderPid(Unit_t* unit, int pid)
{
    UNIT_LEADER_CHARACTER(unit) = pid;
}

inline void SetUnitHp(Unit_t* unit, int value)
{
    unit->curHP = value;
...
inline void AddUnitHp(Unit_t* unit, int amount)
{
    int hp = unit->curHP;
...
void SetUnitStatus(Unit_t* unit, int status)
{
    if (status == 0)
    {
        unit->statusId    = 0;
...
inline char const* GetUnitStatusName(Unit_t* unit)
{
...
int GetUnitSpriteId(Unit_t* unit)
{
    ...
    } // switch (GetTrap(unit->ballistaId)->data[TRAP_D_BLST_ITEMID])
}

i8 UnitAddItem(Unit_t* unit, int item)
{
    int i;
...
inline void UnitRemoveItem(Unit_t* unit, int slot)
{
    unit->items[slot] = 0;
...
i8 UnitHasItem(Unit_t* unit, int item)
{
    int i;
...
i8 CanClassWieldWeaponType(u8 jid, u8 wpnType)
{
...
Unit_t* CreateUnit(const struct UnitInfo* info)
{
    struct UnitInfo buf;
    ...
    if (info->genMonster)
    {
        ...
        if (CanClassWieldWeaponType(monsterClass, ITEM_KIND_BOW) == TRUE)
        {
...
void UnitInitFromInfo(Unit_t* unit, const struct UnitInfo* info)
{
...
void UnitInitItemsFromInfo(Unit_t* unit, const struct UnitInfo* info)
{
    int i;
...
void UnitInitStats(Unit_t* unit, const struct PInfo* character)
{
    int i;
...
void UnitAutolevelWeaponExp(Unit_t* unit, const struct UnitInfo* info)
{
    if (info->autolevel)
    {
        int i;
        ...
        for (i = 0; i < GetUnitItemCount(unit); ++i)
        {
            ...
            if (!(GetItemAttributes(item) & ITEM_ATTR_REQUIRES_WEXP))
                continue;
...
{
    BattleUnit_t tmpBattleUnit;
    short levelsLeft;

    tmpBattleUnit.exp_gained = 0;
...
void UnitCheckStatOverflow(Unit_t* unit)
{
    ...
    if (unit->con_bonus > (UNIT_CON_MAX(unit) - UNIT_CON_BASE(unit)))
        unit->con_bonus = (UNIT_CON_MAX(unit) - UNIT_CON_BASE(unit));

    if (unit->mov_bonus > (UNIT_MOV_MAX(unit) - UNIT_MOV_BASE(unit)))
        unit->mov_bonus = (UNIT_MOV_MAX(unit) - UNIT_MOV_BASE(unit));
}
...
i8 CanUnitCarry(Unit_t* actor, Unit_t* target)
{
...
{
    Unit_t* target = GetUnit(actor->rescue);
...
i8 UnitGiveRescue(Unit_t* actor, Unit_t* target)
{
    Unit_t* rescuee = GetUnit(actor->rescue);
...
inline char const* GetUnitRescueName(Unit_t* unit)
{
    if (!unit->rescue)
        return GetMsg(sStatusNameTextIdLookup[0]);

    return GetMsg(GetUnit(unit->rescue)->pinfo->name_msg);
}
...
{
    Unit_t* newUnit = GetFreeUnit(faction);
...
inline i8 CanUnitCrossTerrain(Unit_t* unit, int terrain)
{
    const i8* lookup = GetUnitMovementCost(unit);
...
void UnitSyncMovement(Unit_t* unit)
{
    if (unit->flags & UNIT_FLAG_RESCUING)
    {
        Unit_t* rescuee = GetUnit(unit->rescue);
    ...
    if (unit->flags & UNIT_FLAG_IN_BALLISTA)
    {
        Trap_t* trap = GetTrap(unit->ballistaId);
...
void UnitGetDropPositionOnDeath(Unit_t* unit, int* xOut, int* yOut)
{
    ...
    MapFloodExtended(unit->x, unit->y, gUnknown_0880BB96);
...
i8 UnitKnowsMagic(Unit_t* unit)
{
    u8 combinedRanks = 0; 

    combinedRanks |= unit->wexp[ITEM_KIND_STAFF];
    combinedRanks |= unit->wexp[ITEM_KIND_ANIMA];
    combinedRanks |= unit->wexp[ITEM_KIND_LIGHT];
    combinedRanks |= unit->wexp[ITEM_KIND_DARK];
...
int GetUnitKeyItemSlotForTerrain(Unit_t* unit, int terrain)
{
    ...
    case TERRAIN_CHEST_21:
        slot = FindUnitItemSlot(unit, IID_CHESTKEY);

        if (slot < 0)
            slot = FindUnitItemSlot(unit, IID_CHESTKEY_BUNDLE);
...
i8 IsPositionMagicSealed(int x, int y)
{
    int i;
...
i8 IsUnitMagicSealed(Unit_t* unit)
{
...
const i8* GetUnitMovementCost(Unit_t* unit)
{
    if (unit->flags & UNIT_FLAG_IN_BALLISTA)
...
i8 IsUnitSlotAvailable(int faction)
{
    int i;
