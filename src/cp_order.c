...
static int GetUnitBattleAiPriority(Unit_t* unit);
static int GetUnitAiPriority(Unit_t* unit);
...
struct ProcScr CONST_DATA ProcScr_CpOrder[] =
{
    PROC_NAME("E_CPORDER"),
...
struct ProcScr CONST_DATA ProcScr_BerserkCpOrder[] =
{
...
void CpOrderBerserkInit(ProcPtr proc)
{
    ...
    {
        Unit_t* unit = GetUnit(faction + i + 1);
    ...
    if (aiNum != 0)
    {
        ...
        SpawnProcLocking(ProcScr_CpDecide, proc);
    }
}
...
int GetUnitBattleAiPriority(Unit_t* unit)
{
    int i, item;
...
int GetUnitAiPriority(Unit_t* unit)
{
