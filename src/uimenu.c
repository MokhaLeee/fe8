...
Proc_t* EndMenu(struct MenuProc* proc)
{
    ...
    if (item->def->on_switch_out)
        item->def->on_switch_out(proc, item);

    if (proc->def->on_end)
        proc->def->on_end(proc);
...
{
    if (proc->def->on_init)
        proc->def->on_init(proc);

    if (proc->menuItems[proc->itemCurrent]->def->on_switch_in)
        proc->menuItems[proc->itemCurrent]->def->on_switch_in(proc, proc->menuItems[proc->itemCurrent]);
}
...
void Menu_HandleDirectionInputs(struct MenuProc* proc)
{
    ...
    {
        if (proc->menuItems[proc->itemPrevious]->def->on_switch_out)
            proc->menuItems[proc->itemPrevious]->def->on_switch_out(proc, proc->menuItems[proc->itemPrevious]);
...
struct MenuProc* StartMenu_AndDoSomethingCommands(
    const struct MenuInfo* def, int xSubject, int xTileLeft, int xTileRight)
{
