...
typedef struct ProcScr ProcScr_t;
...
#define PROC_HEADER                                                                        \
    ProcScr_t const* proc_script; /* pointer to proc script */                        \
    ProcScr_t const* proc_scrCur; /* pointer to currently executing script command */ \
...
void InitProcs(void);
ProcPtr SpawnProc(ProcScr_t const* script, ProcPtr parent);
ProcPtr SpawnProcLocking(ProcScr_t const* script, ProcPtr parent);
void Proc_End(ProcPtr proc);
void Proc_Run(ProcPtr proc);
void Proc_Break(ProcPtr proc);
ProcPtr FindProc(ProcScr_t const* script);
void Proc_Goto(ProcPtr proc, int label);
void Proc_GotoScript(ProcPtr proc, const ProcScr_t* script);
void Proc_SetMark(ProcPtr proc, int mark);
void Proc_SetEndCb(ProcPtr proc, ProcFunc func);
void ForEveryProc(ProcFunc func);
void ForEachProc(ProcScr_t const* script, ProcFunc func);
...
void EndEachProc(ProcScr_t const* script);
void BreakEachProc(ProcScr_t const* script);
...
// ??? func_080034D4(???);
// ??? func_080034FC(???);
void BeginFindProc(struct ProcFindIterator* it, ProcScr_t const* script);
